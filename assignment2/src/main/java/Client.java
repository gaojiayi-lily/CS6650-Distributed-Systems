...
public class Client {
    ...
    public static String BASE_PATH = "http://" + serverAddress+ ":" + portNumber + "/assignment2_war_exploded/assignment2";
    ...
    public static synchronized void main(String[] args) throws ParseException, InterruptedException, IOException {
        ...
        BASE_PATH = "http://" + serverAddress+ ":" + portNumber + "/assignment2_war_exploded/assignment2";
        ...
        executor.startNewStoreThread(threadPoolEast, maxStores / 4, purchaseRecord);

        // TODO: Change the Thread.sleep() function -> not efficient
        while (requestCount.get() <= (maxStores / 4) * numberPurchase * 3) {
            Thread.sleep(1);
        }
        executor.startNewStoreThread(threadPoolMid, maxStores / 4, purchaseRecord);

        while (requestCount.get() <= (maxStores / 4) * numberPurchase * (3 + 2 + 2)) {
            Thread.sleep(1);
        }
        executor.startNewStoreThread(threadPoolWest, maxStores / 2, purchaseRecord);
